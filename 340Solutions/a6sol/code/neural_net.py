...
class NeuralNet:
    ...
    def __init__(self, fun_obj, optimizer, encoder, predictor, classifier_yes=False):
        """
        fun_obj will be a composition of two function objects: 
    ...
    def fit(self, X, y):
        ...
        # Initial guess
        w_init = np.concatenate([self.encoder.get_parameters_flattened(), self.predictor.get_parameters_flattened()])
        f, g = self.fun_obj.evaluate(w_init, X, y)
        ...
        for i in range(1000):
            ...
            if break_yes:  # in reality, convergence will rarely happen, so we will ignore break conditions
                break
    ...
    def get_parameters_flattened(self):
        return np.concatenate([self.encoder.get_parameters_flattened(), self.predictor.get_parameters_flattened()])
    ...
    def predict(self, X):
        ...
        """
        Z, _ = self.encoder.encode(X)  # presumably MultiLayerEncoder, which also returns activation trajectory.
